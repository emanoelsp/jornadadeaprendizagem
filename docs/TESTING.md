...
- `src/features/enade/data.test.ts`: valida cálculo de acerto médio, identificação de UCs de alto risco e vínculos básicos entre questão, competência e unidade curricular.
- `src/features/enade/data.test.ts`: valida diagnóstico por turma, geração de jornada com três macros e normalização de risco.
